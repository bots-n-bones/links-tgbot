...
async def _make_daily_digest(db_session) -> Collection:
    ...
    c = Collection(
        ...
        summary_md="",
        articles=[{"title": "Fresh find", "url": "https://b.com", "description": "hot off the press"}],
...
async def test_digest_page_lists_weekly_digest(db_session):
...
async def test_digest_page_lists_both_daily_and_weekly_together(db_session):
    await _make_daily_digest(db_session)
    await _make_weekly_digest(db_session)
    with TestClient(app) as client:
        resp = client.get("/digest")
    assert resp.status_code == 200
...
async def test_digest_detail_page_shows_articles(db_session):
    c = await _make_weekly_digest(db_session)
    with TestClient(app) as client:
        resp = client.get(f"/digest/{c.id}")
    assert resp.status_code == 200
...
async def test_digest_detail_page_404_for_wrong_theme(db_session):
    c = await _make_collection(db_session)  # theme="ai", не daily/weekly-digest
    with TestClient(app) as client:
        resp = client.get(f"/digest/{c.id}")
    assert resp.status_code == 404


async def test_old_weekly_digest_url_redirects_to_digest(db_session):
    c = await _make_weekly_digest(db_session)
    with TestClient(app) as client:
