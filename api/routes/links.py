...
async def query_top_links(session: AsyncSession, *, days: int = 7, limit: int = 5) -> list[Link]:
    """F-55: топ-5 по priority_score среди ссылок с активностью за последние N дней."""
    cutoff = datetime.now(UTC) - timedelta(days=days)
    recent_link_ids = select(LinkSource.link_id).where(LinkSource.created_at >= cutoff)
    stmt = (
        select(Link)
        .where(Link.is_hidden.is_(False), Link.id.in_(recent_link_ids))
...
@router.patch("/{link_id}/tags")
async def update_tags(link_id: int, request: Request, tags: str = Form(...)):
    sessionmaker = get_sessionmaker()
    async with sessionmaker() as session:
        link = await session.get(Link, link_id)
        if link is None:
